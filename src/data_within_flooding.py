...
QUERY_DICT = OrderedDict([
    ...
    ("Parcels within the FEMA SFHA and the Community Floodplain", 0),
    ("Buildings within the FEMA SFHA and the Community Floodplain", 0),
    ("Buildings within the FEMA SFHA and the Community Floodplain > 600 square feet", 0),
    ("Parcels within the FEMA SFHA or the Community Floodplain", 0),
    ("Buildings within the FEMA SFHA or the Community Floodplain", 0),
    ("Buildings within the FEMA SFHA or the Community Floodplain > 600 square feet", 0)   
])
...
def dataset_queries(dataset, prefix):
    ...
    selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "fema_sfha = 'T'")
    ...
    selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "community_flooding = 'T'")

    QUERY_DICT[f"{prefix} within the Community Floodplain"] = \
        int(arcpy.GetCount_management(selected).getOutput(0))

    # dataset within the FEMA SFHA and the Community Floodplain
    selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "fema_sfha = 'T' AND community_flooding = 'T'")

    QUERY_DICT[f"{prefix} within the FEMA SFHA and the Community Floodplain"] = \
        int(arcpy.GetCount_management(selected).getOutput(0))

    # dataset within the FEMA SFHA or the Community Floodplain
    selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "fema_sfha = 'T' OR community_flooding = 'T'")

    QUERY_DICT[f"{prefix} within the FEMA SFHA or the Community Floodplain"] = \
        int(arcpy.GetCount_management(selected).getOutput(0))    

    if prefix == "Buildings":
        # dataset within the FEMA SFHA > 600 square feet
        selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "fema_sfha = 'T' AND sq_ft > 600") 

        QUERY_DICT[f"{prefix} within the FEMA SFHA > 600 square feet"] = \
            int(arcpy.GetCount_management(selected).getOutput(0))

        # dataset within the Community Floodplain > 600 square feet
        selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "community_flooding = 'T' AND sq_ft > 600")
        ...
        selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "fema_sfha = 'T' AND community_flooding = 'T' AND sq_ft > 600")

        QUERY_DICT[f"{prefix} within the FEMA SFHA and the Community Floodplain > 600 square feet"] = \
            int(arcpy.GetCount_management(selected).getOutput(0))

        # dataset within the FEMA SFHA or the Community Floodplain > 600 square feet
        selected = arcpy.SelectLayerByAttribute_management(dataset, "NEW_SELECTION", "(fema_sfha = 'T' OR community_flooding = 'T') AND sq_ft > 600")

        QUERY_DICT[f"{prefix} within the FEMA SFHA or the Community Floodplain > 600 square feet"] = \
            int(arcpy.GetCount_management(selected).getOutput(0))        
