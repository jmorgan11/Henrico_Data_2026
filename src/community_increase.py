...
def main(parcels, buildings):
    ...
    try:
        ...
        selected = arcpy.SelectLayerByAttribute_management(parcels, "NEW_SELECTION", "com_increase = 'T'")
        QUERY_DICT["Parcels with increasing Community Floodplains areas"] = int(arcpy.GetCount_management(selected).getOutput(0))

        # Buildings with increasing community flooding
        selected = arcpy.SelectLayerByAttribute_management(buildings, "NEW_SELECTION", "com_increase = 'T'")
        QUERY_DICT["Buildings with increasing Community Floodplains areas"] = int(arcpy.GetCount_management(selected).getOutput(0))

        # Buildings with increasing community flooding and > 600 square feet
        selected = arcpy.SelectLayerByAttribute_management(buildings, "NEW_SELECTION", "com_increase = 'T' And sq_ft >= 600")
        QUERY_DICT["Buildings with increasing Community Floodplains areas > 600 square feet"] = int(arcpy.GetCount_management(selected).getOutput(0))
